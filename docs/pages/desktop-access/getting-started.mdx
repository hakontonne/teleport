...
This guide will help you configure Teleport and your Active Directory domain controller to communicate securely with each other, register available desktops with your Teleport cluster, and provide secure access to them.
...
- A host where you will run the Teleport Desktop Service.
  ...
  of Teleport running your Auth and/or Proxy service(s). To install Desktop Access into an existing Teleport instance running other services, see the [Manual Setup](manual-setup.mdx) guide for Desktop Access.</ScopedBlock>
...
In your web browser, access the teleport Web UI at <ScopedBlock scope={["oss", "enterprise"]}>`teleport.example.com`</ScopedBlock><ScopedBlock scope={["cloud"]}>`mytennant.teleport.sh`</ScopedBlock>. Click on your user name at the top right and select **Manage Access**, Select "Desktop" from the main menu, then **NEXT**:

If your Windows server is already running as an Active Directory controller, skip to the next step. Otherwise, copy and paste the first command provided into a Windows PowerShell window. If your server is not already running AD Certificate Services, copy and paste the second command after the first one completes and the server restarts:
...
On the host you installed Teleport to run as the Desktop Access connector, edit `/etc/teleport.yaml` and paste the configuration provided by the output of the previous step:
